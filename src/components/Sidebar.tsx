...
const navItemsByRole: Record<UserRole, { label: string, href: string, icon: any }[]> = {
  ...
  manager: [
    ...
    { label: "Mail", href: "/manager/mail", icon: Mail },
...
export default function Sidebar({ role, userName, isCompact = false, onToggleCompact, isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed left-0 top-0 h-screen transition-all duration-500 ease-in-out bg-background border-r border-secondary flex flex-col z-[70] overflow-hidden",
        isCompact ? "w-20" : "w-72",
        "lg:translate-x-0",
        ...
        <div className={cn("p-6 flex items-center justify-between", isCompact ? "flex-col gap-8" : "flex-row")}>
          <div className="flex items-center gap-3 group">
            ...
            {!isCompact && (
              <div className="animate-in fade-in slide-in-from-left-2 duration-500">
                <h1 className="text-lg font-bold tracking-tight leading-none">PIVOT SYSTEM</h1>
              </div>
            )}
          </div>
          ...
          <button
            ...
            className={cn(
              "p-2 rounded-lg hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground hidden lg:block",
          ...
          {items.map((item) => {
            ...
            return (
              ...
              >
                ...
                {!isCompact && (
                  <span className="font-bold tracking-wider text-xs uppercase animate-in fade-in slide-in-from-left-2 duration-500">{item.label}</span>
                )}
                {isActive && !isCompact && (
          ...
          <div className={cn("flex flex-col gap-2 w-full", isCompact ? "items-center" : "")}>
            <div className={cn("grid gap-2 w-full", isCompact ? "grid-cols-1" : "grid-cols-2")}>
              <button
                ...
                className={cn(
                  "flex items-center justify-center gap-2 p-3 rounded-lg bg-background border border-secondary hover:border-accent transition-all text-xs font-bold uppercase tracking-wider shadow-sm",
              ...
              <button
                ...
                className={cn(
                  "flex items-center justify-center gap-2 p-3 rounded-lg bg-background border border-secondary hover:border-red-500 hover:text-red-500 transition-all text-xs font-bold uppercase tracking-wider shadow-sm",
                  isCompact ? "h-10 w-10 p-0 mx-auto" : ""
                )}
              >
