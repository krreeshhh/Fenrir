...
export default function ProfilePage() {
   ...
   // Populate edit fields once user data loads (only once)
   ...
   const handleSaveChanges = async () => {
      ...
      if (error) {
         ...
         refreshName(newFullName); // update context so sidebar/header updates too
         setSaved(true);
   ...
   const handleCalendarSync = async () => {
      ...
      setSyncDone(true); setTimeout(() => setSyncDone(false), 3000);
      setSyncing(false);
   };

   const initials = editFirst.slice(0, 1).toUpperCase() || email.slice(0, 1).toUpperCase() || "?";
   const fullDisplayName = `${editFirst} ${editLast}`.trim() || fullName || "Employee";
         ...
         <div className="space-y-6 pb-16 max-w-5xl mx-auto">
            ...
            <div className="flex items-center justify-between">
               ...
               <button onClick={handleSignOut}
                  className="flex items-center gap-2 px-4 py-2 bg-red-500/10 text-red-500 border border-red-500/20 rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-red-500/20 transition-all">
            ...
            <div className="bg-background border border-secondary rounded-2xl overflow-hidden shadow-sm">
               <div className="h-16 relative">
                  <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-accent/10 to-transparent" />
               </div>
               <div className="px-6 pb-6 -mt-10 relative">
                  <div className="flex items-end gap-4">
                     <div className="relative group">
                        <div className="h-20 w-20 rounded-2xl bg-foreground text-background font-black text-3xl flex items-center justify-center border-4 border-background shadow-xl">
                           {initials}
                        </div>
                        <button className="absolute -bottom-1 -right-1 h-7 w-7 rounded-lg bg-accent text-white flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               <div className="lg:col-span-2 space-y-5">
                  <div className="bg-background border border-secondary rounded-2xl p-6">
                     ...
                     <div className="mt-6 flex items-center gap-3 pt-4 border-t border-secondary">
                        <button onClick={handleSaveChanges} disabled={saving}
                           className="px-6 py-2.5 bg-foreground text-background rounded-xl font-black text-xs uppercase tracking-widest hover:bg-accent transition-all flex items-center gap-2 disabled:opacity-50">
                  ...
                  <div className="bg-background border border-secondary rounded-2xl p-6">
                     <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground mb-5 flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-accent" /> Integrations
                     </h3>
                     <div className="flex items-center justify-between p-4 bg-secondary/30 border border-secondary rounded-xl">
                        ...
                        <button onClick={handleCalendarSync} disabled={syncing}
                           className={cn("px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-widest transition-all flex items-center gap-1.5",
                              syncing ? "bg-secondary text-muted-foreground" : "bg-foreground text-background hover:bg-accent")}>
               ...
               <div className="space-y-5">
                  <div className="bg-foreground text-background rounded-2xl p-6 space-y-4">
                     ...
                     <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-green-400 bg-green-500/10 rounded-lg px-2.5 py-1.5 w-fit">
                        <CheckCircle2 className="h-3 w-3" /> Session Encrypted
                     </div>
                  </div>
                  ...
                  <div className="bg-background border border-secondary rounded-2xl p-6 space-y-4">
                     ...
                     <div className="w-full h-2 bg-secondary rounded-full overflow-hidden">
                        <div className="h-2 rounded-full bg-gradient-to-r from-accent to-yellow-400 transition-all duration-1000" style={{ width: `${Math.min((score / 5000) * 100, 100)}%` }} />
                     </div>
...
function ProfileField({ label, value, onChange, icon: Icon, disabled = false }: any) {
   return (
      <div className="space-y-1.5">
         ...
         <div className={cn("flex items-center gap-3 px-4 py-3 bg-secondary/20 border rounded-xl transition-all",
            ...
            <input type="text" value={value} disabled={disabled}
               ...
               className="bg-transparent border-none outline-none flex-1 text-sm font-bold w-full" />
         </div>
      </div>
   );
}
