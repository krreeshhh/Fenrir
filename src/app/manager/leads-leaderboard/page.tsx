...
import { Trophy, ArrowUp, ArrowDown, Minus, Star, ShieldCheck, Loader2 } from "lucide-react";
         ...
         <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {leads.slice(0, 3).map((u, i) => (
               <div key={u.id} className={cn(
                  ...
                  i === 0 ? "border-accent/40 bg-accent/5 ring-1 ring-accent/20" : "border-secondary"
               )}>
                  {i === 0 && <div className="absolute top-0 right-0 p-2"><Trophy className="h-4 w-4 text-yellow-500" /></div>}
                  <div className={cn(
                     "h-10 w-10 rounded-lg mx-auto flex items-center justify-center font-black text-sm mb-2",
                     i === 0 ? "bg-accent text-white" : i === 1 ? "bg-foreground text-background" : "bg-secondary border-2 border-foreground text-foreground"
                  )}>{u.full_name[0]}</div>
                  <p className="text-sm font-bold truncate">{u.full_name}</p>
                  <p className="text-xs font-black text-accent">{u.score.toLocaleString()}</p>
                  <p className="text-[9px] text-muted-foreground uppercase tracking-widest mt-0.5">Rank #{i + 1}</p>
               </div>
            ))}
         </div>
            ...
            {leads.map(user => (
               <div key={user.id} className="flex items-center justify-between p-4 hover:bg-secondary/10 transition-colors group">
                  <div className="flex items-center gap-4">
                     ...
                     <div className={cn(
                        "h-9 w-9 rounded-lg flex items-center justify-center font-black text-sm border-2 transition-all",
