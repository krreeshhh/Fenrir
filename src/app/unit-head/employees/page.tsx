      ...
      <div className="space-y-6 pb-16">
         <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
               ...
               <h2 className="text-xl font-black uppercase tracking-tight">Personnel Network Hub</h2>
            </div>
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-background border border-accent/40 rounded-xl p-5 shadow-sm transition-all hover:scale-[1.02]">
               <div className="flex items-center gap-2 mb-2"><Users className="h-3.5 w-3.5 text-accent" /><p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Active Nodes</p></div>
               <p className="text-2xl font-black tracking-tighter">{employees.length}</p>
            </div>
            <div className="bg-background border border-secondary rounded-xl p-5 shadow-sm transition-all hover:scale-[1.02]">
               <div className="flex items-center gap-2 mb-2"><Activity className="h-3.5 w-3.5 text-muted-foreground" /><p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Network Load</p></div>
               <p className="text-2xl font-black tracking-tighter">74%</p>
            </div>
            <div className="grid grid-cols-1 gap-1">
                <div className="bg-background border border-secondary rounded-xl p-5 shadow-sm transition-all hover:scale-[1.02]">
                <div className="flex items-center gap-1.5 mb-2"><TrendingUp className="h-3.5 w-3.5 text-accent" /><p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">Efficiency</p></div>
                <p className="text-2xl font-black tracking-tighter">9.2</p>
                </div>
            </div>
            <div className="bg-background border border-secondary rounded-xl p-5 shadow-sm transition-all hover:scale-[1.02]">
               <div className="flex items-center gap-2 mb-2"><ShieldCheck className="h-3.5 w-3.5 text-green-500" /><p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">System Sync</p></div>
               <p className="text-2xl font-black text-green-500 uppercase tracking-tighter">SECURED</p>
            </div>
         </div>
         ...
         <div className="bg-background border border-secondary rounded-xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-secondary flex items-center justify-between bg-secondary/5">
               <div className="flex items-center gap-3 bg-background border border-secondary rounded-lg px-4 py-2 shadow-inner">
                  ...
                  <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} type="text" placeholder="FILTER PERSONNEL NODE..." className="bg-transparent text-[10px] font-black uppercase outline-none w-48 placeholder:text-muted-foreground/30 tracking-widest" />
               </div>
               <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">{filtered.length} nodes connected</span>
            </div>
            <table className="w-full text-left">
               <thead className="bg-secondary/10 border-b border-secondary">
                  <tr>
                     <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground">Identity</th>
                     <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground">Functional Authority</th>
                     <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground hidden md:table-cell">Operational Cluster</th>
               ...
               <tbody className="divide-y divide-secondary">
                  {filtered.length === 0 ? (
                     <tr><td colSpan={5} className="p-16 text-center text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-30">No matching personnel nodes recorded in system</td></tr>
                  ) : filtered.map(emp => (
                     <tr key={emp.id} className="hover:bg-accent/5 transition-all group">
                        <td className="px-6 py-5">
                           <div className="flex items-center gap-4">
                              <span className="text-[10px] font-black text-muted-foreground/30 w-4">{emp.rank}</span>
                              ...
                              <div>
                                 ...
                                 <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5 opacity-60">NODE: PVT-ID-{emp.id.slice(0, 4).toUpperCase()}</p>
                              </div>
                           </div>
                        </td>
                        <td className="px-6 py-5">
                           {emp.role === 'admin' ? (
                               <div className="flex items-center gap-2 px-3 py-1.5 bg-accent/10 border border-accent/20 rounded-lg w-fit">
                                    ...
                                    <span className="text-[9px] font-black text-accent uppercase tracking-widest">SYSTEM ADMIN</span>
                               </div>
                           ) : (
                               <select 
                                 value={emp.role} 
                        ...
                        <td className="px-6 py-5 hidden md:table-cell">
                           <div className="space-y-2">
                              <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest mb-1 truncate max-w-[140px]">{emp.project}</p>
                        ...
                        <td className="px-6 py-5 text-right">
                           ...
                           <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest opacity-40">SYSTEM POINTS</p>
                        </td>
                        <td className="px-6 py-5 text-right">
